...
select emp_id, emp_name, concat(emp_name,"/",eng_name) eng_name, hire_date, phone, salary 
from employee;
...
select substring("대한민국 홍길동", 1,6), 
		substring("대한민국 홍길동", 6,3)
from dual; -- 저장소관련은 0번째부터 지금은 저장소가 아니여서 1번째


-- 사원테이블의 사번, 사원명, 입사년도, 입사월, 입사일, 급여를 조회

select emp_id,emp_name, hire_date,substring(hire_date,1,4) as year, substring(hire_date,6,2) as month, substring(hire_date,9,2) as day
from employee;
...
select substring(hire_date,1,4), emp_name
from employee
...
-- 영어 이름은 전체 대문자, 부서아이디는 소문자, 이메일은 대문자
select emp_id,emp_name, upper(eng_name) as eng_name, lower(dept_id) as dept_id, upper(email) as email, salary from employee;

-- (5) trim() : 공백 제거
select trim('       대한민국') as t1,
	   trim('대한민국       ') as t2,
       trim('대한       민국') as t3, -- 중간 공백은 지울 수 없음
       trim('   대한민국   ') as t4
from dual;
...
-- 사번 기준으로 내림차순 정렬 
select emp_id, emp_name, hire_date, phone, format(ifnull(salary, 0),0) as salary, format(ifnull(salary*0.2,0),0) as bonus
from employee
...
select now() as date, 
	   ...
	   cast(curdate() as datetime) as datetime from dual;
...
select emp_id,
	   emp_name,
       replace(cast(hire_date as char),'-','/') as hire_date, -- 원래는 replace는 안에 문자열만 받기때문에 cast형변환 필요
...
from employee -- 그룹함수가 아닌것들은 같이 정렬할 수 없다
group by emp_id; -- 유니크한 것들하고 그룹함수는 같이쓰지않는게좋다
...
-- 3자리 구분, 마지막 '만원' 표시
select concat(format(avg(salary),0), '만원') as 평균급여, concat(format(sum(salary),0), '만원') as 총급여
from employee
where dept_id = 'sys';
...
-- [group by] : 그룹함수와 일반컴럼을 함께 사용할 수 있도록 함
-- where절까지는 하나다
